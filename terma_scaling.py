"""Differentiable TERMA source scaling for the multislab pencil-beam engine.
...
class TermaScalingLayer(nn.Module):
    ...
    ``spacing_mm`` follows this repository's patient-grid convention
    ``(rH, rD, rW)``. The BEV density order is ``[B,G,D,H,W]``, hence the
